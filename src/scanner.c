...
bool tree_sitter_haml_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
      ...
      if (lexer->lookahead == '\\') {
        ...
        if (is_bytes(delimiter)) {
          ...
          if (lexer->lookahead == 'N' || lexer->lookahead == 'u' || lexer->lookahead == 'U') {
            ...
            // https://docs.slim.org/3/reference/lexical_analysis.html#string-and-bytes-literals
            advance(lexer);
          } else {
            lexer->result_symbol = STRING_CONTENT;
            return has_content;
  ...
  if (found_end_of_line) {
    if (scanner->indents.size > 0) {
      ...
      bool next_tok_is_string_start = lexer->lookahead == '\"' || lexer->lookahead == '\'' || lexer->lookahead == '`';

      if ((valid_symbols[DEDENT] || (!valid_symbols[NEWLINE] &&
                                     !(valid_symbols[STRING_START] && next_tok_is_string_start) && !within_brackets)) &&
